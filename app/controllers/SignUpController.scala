  ...
  def submit: Action[AnyContent] = UnsecuredAction.async { implicit request: Request[AnyContent] =>
    SignUpForm.form.bindFromRequest().fold(
      ...
      data => {
        ...
        val id = splittedEmail.head
        val domain = splittedEmail.last
        val result = Redirect(routes.SignUpController.view).flashing("info" -> Messages("sign.up.email.sent", id, domain))
        val loginInfo = LoginInfo(CredentialsProvider.ID, data.email)
        userService.retrieve(loginInfo).flatMap {
          case Some(user) =>
            val url = Calls.signin.absoluteURL(secure = true)
          ...
          case None =>
            ...
            val user = User(
              ...
              organization = Some(data.organization),
              roleTitle = Some(data.roleTitle),
            ...
            for {
              ...
              mailerClient.send(Email(
                ...
                bodyText = Some(views.txt.emails.signupNotify(user).body),
                bodyHtml = Some(views.html.emails.signupNotify(user).body)
              ))
